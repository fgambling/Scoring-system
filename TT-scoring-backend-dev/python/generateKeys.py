...
spelling_variants = {'accessorize': 'accessorise',
...
# Reverse mapping
variant_reverse = {v: k for k, v in spelling_variants.items()}
def get_derivational_forms(word):
    """Retrieve derivational forms from WordNet"""
    derivational_forms = set()
    for lemma in wn.lemmas(word):  # Find lemmas of the word
        for related_form in lemma.derivationally_related_forms():
            derivational_forms.add(related_form.name().replace('_', ' '))
    return list(derivational_forms)

def get_wordnet_pos(treebank_tag):
    """Convert treebank POS tags to a WordNet compatible UPOS format."""
    if treebank_tag.startswith('J'):
        ...
        return 'NOUN'  # Default to NOUN if uncertain
...
def generate_alternatives(sentence):
    ...
    for word, tag in pos_tags:
        alternatives = set()
        upos = get_wordnet_pos(tag)  # Get the WordNet UPOS

        # Get all inflections for the word based on its part of speech
        inflections = lemminflect.getAllInflections(word, upos=upos)
        for infl_list in inflections.values():
            alternatives.update(infl_list)

        # Get derivational forms from WordNet
        deriv_forms = get_derivational_forms(word)
        alternatives.update(deriv_forms)

        # Check for spelling variants
        word_lower = word.lower()
        if word_lower in spelling_variants:
            alternatives.add(spelling_variants[word_lower])
        if word_lower in variant_reverse:
            alternatives.add(variant_reverse[word_lower])

        # Check if there are any alternatives different from the original word
        alternatives.discard(word)  # Make sure the original word is not in the alternatives
        if alternatives:  # Only add if there are other alternatives
            alternative_keys[word] = list(alternatives)

    return {
        "key": sentence,
        "alternativeKeys": alternative_keys
    }

# Example usage
# sentences = ["How are you", "I need to colourize the image.", "I like music"]
# keys_with_alternatives = [generate_alternatives(sentence) for sentence in sentences]
def main():
    # read json
